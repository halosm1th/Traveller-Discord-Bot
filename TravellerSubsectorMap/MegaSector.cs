...
namespace Traveller_subsector_generator
{
    class MegaSector
    {
        ...
        public void GenerateAndWriteWriteMegaSector(string path)
        {
            foreach (var supers in supersectors)
            {
                supers.GenerateSuperSector();

                var secPath = path + $"\\{supers.Name}_SuperSector\\";
                Directory.CreateDirectory(secPath);
                
                //Console.WriteLine("writing supersector");
                supers.WriteSuperSector(secPath);
            }
        }
    }
}