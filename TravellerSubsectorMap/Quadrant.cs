...
namespace TravellerSubsectorMap
{
    class Quadrant
    {
        ...
        public void GenerateAndWriteQuadrants(string path)
        {
            for (int y = 0; y < megaSectors.GetLength(0); y++)
            {
                for (int x = 0; x < megaSectors.GetLength(1); x++)
                {
                    ...
                    var megSecPath = path + $"\\{mega.Name}_MegaSector\\";
                    Directory.CreateDirectory(megSecPath);
                    //Console.WriteLine($"Writing {mega.Name} to {megSecPath} now.");
                    mega.GenerateAndWriteWriteMegaSector(megSecPath);
